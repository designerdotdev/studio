...
class SeedSampleData extends Command
{
    ...
    public function handle(ComponentRepository $components, PageRepository $pages): int
    {
    ...
    <div class="container mx-auto px-4 text-center">
        ...
        foreach ($sampleComponents as $data) {
            if (!$components->find($data['name'])) {
                $components->create($data);
                $this->line("  Created component: {$data['name']}");
            } else {
                $this->line("  Component exists: {$data['name']}");
            }
        }
