...
- `components/layouts/app.blade.php` — editor shell laid out as three columns: a full-height 56px **rail** (40px buttons) (`s-rail`, no top border) holding the logo/menu button (the `menu` slot, dropdown opens to the right) then Assistant (dev mode) / Sections / Pages / Content / Media; the 320px sidebar whose panel header rows are `h-11` (44px, like the Sites toolbar) so they sit on the topbar line; and the main column — a **workspace card** (`s-workspace`: rounded, bordered, `bg-panel`, inset from the shell) holding the topbar and the canvas; the rail and sidebar share the plain `bg-shell` with no dividers. A desktop preview fills the card flush (`s-frame.is-flush`); tablet/mobile previews sit on a dotted `s-canvas.is-device` surface. `$store.studio.rail` + `setRail(name, force)` (same item again collapses the panel; persisted in localStorage `studio.rail`; fires the `studio:rail` window event). `home.blade.php` mounts the five panels in the sidebar slot, one visible at a time; selecting on the canvas forces the Sections panel. The sidebar collapses via `$store.studio.sidebar` (localStorage `studio.sidebar`); the toggle is the first item of the topbar, aligned with the canvas edge. **Themes:** dark is the default; `$store.studio.toggleTheme()` (menu → Light mode) sets `html.studio-light`, persisted in localStorage `studio.theme` and applied by an inline head script before first paint. All chrome colours are `@theme` tokens overridden under `html.studio-light` in `studio.css`; hover tints use the `wash` / `wash-strong` / `wash-faint` tokens instead of `bg-white/N` so they flip with the theme
