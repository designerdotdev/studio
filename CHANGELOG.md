...
- **One layout, the DevDojo builder's.** An activity rail (menu, then Assistant · Sections · Pages · Content · Media) and a resizable sidebar on the ground, and the site in a content card whose header holds the sidebar toggle, the Preview / Edit / Code pill (only the active segment carries its label), the page pill in the centre, a device button that cycles Desktop → Tablet → Phone, the open-in-new-tab link and Publish. Every dropdown is dark. The floating/pinnable dock, its View switches, the five Workspace presets, the joined composer, the floating chat, and the popover/sheet panel frames are gone; nothing about where the chrome sits is a setting any more.
