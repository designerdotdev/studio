...
>
    ...
    <div class="flex gap-1.5">
        ...
        <button
            type="button"
            class="s-btn-outline shrink-0 !px-2.5"
            title="Choose from the media library"
            x-on:click="window.Studio.mediaPick().then((url) => { if (!url) return; preview('{{ $sectionId }}', '{{ $key }}', url); $wire.setVariable('{{ $sectionId }}', '{{ $key }}', url); })"
        >
        ...
        <label class="s-btn-outline relative shrink-0 cursor-pointer !px-2.5" :class="uploading && 'pointer-events-none opacity-60'" title="Upload an image">
            ...
            <x-studio::mark-loader x-show="uploading" x-cloak class="h-4 w-4"/>
        </label>
    </div>
</div>
