{{-- Code mode's file tree. The node list arrives flat from
     CodeWorkspace::tree() (path/name/type/depth/parent/design/canonical), so
     this renders as one x-for with indentation instead of a recursive
     include — folder open/close is a plain map in $store.code.

     Two views: Design (only what Studio renders) and Laravel (the whole app,
     with design folders tinted). A design file reached through the Laravel
    ...
    <div class="shrink-0 border-b border-line px-3 pb-2">
        <div class="s-seg !h-7 w-full">
            <button
                ...
                :class="$store.code.view === 'design' && 'is-active'"
                @click="$store.code.setView('design')"
                title="Only the files Studio renders from"
            >
    ...
    <div class="min-h-0 flex-1 overflow-y-auto px-1.5 pb-3 pt-1">
        <template x-for="node in $store.code.visibleNodes" :key="node.path">
            <div class="group relative flex items-center rounded-md transition-colors"
                :class="$store.code.active === (node.canonical || node.path) ? 'bg-wash-strong text-ink' : 'text-soft hover:bg-wash hover:text-ink'">

                {{-- Folder. A design folder keeps its accent tint in the
                     Laravel view, where it is the thing you came looking for. --}}
                <button
                ...
                <button
                    ...
                    @click="$store.code.openFile(node.canonical || node.path)"
                >
                    <span class="truncate font-mono text-[11.5px]" :class="node.design && 'text-ink/90'" x-text="node.name"></span>
                    <span x-show="$store.code.dirty[node.canonical || node.path]" x-cloak class="ml-auto h-1.5 w-1.5 shrink-0 rounded-full bg-accent" title="Unsaved changes"></span>
                </button>
                ...
                <div x-show="node.type === 'file' && $store.code.confirmDelete === node.path" x-cloak class="flex shrink-0 items-center gap-1 pr-1.5" @click.stop>
                    <button class="rounded bg-danger px-1.5 py-0.5 text-[10px] font-semibold text-white" @click="$store.code.deleteFile(node.canonical || node.path)">Delete</button>
                    <button class="rounded px-1.5 py-0.5 text-[10px] font-medium hover:bg-wash" @click="$store.code.confirmDelete = null">Cancel</button>
                </div>

                <button
                    x-show="node.type === 'file' && (node.canonical || node.path).startsWith('designer/') && $store.code.confirmDelete !== node.path"
                    x-cloak
                    type="button"
