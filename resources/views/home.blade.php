...
<x-studio::layouts.app :open-url="$openUrl" :open-label="$openLabel">
        ...
        <script>
            ...
            document.addEventListener('alpine:init', () => {
                ...
                Alpine.store('studio', {
                    ...
                    // Which panel the sidebar shows. The Assistant is a rail
                    // panel like the others, but only in developer mode.
                    rail: (s => ['sections', 'pages', 'content', 'media', 'assistant'].includes(s) && !(s === 'assistant' && !(devModeAvailable && localStorage.getItem('studio.devmode') !== '0')) ? s : 'sections')(localStorage.getItem('studio.rail')),
                    setRail(name, force = false) {
                        if (!['sections', 'pages', 'content', 'media', 'assistant'].includes(name)) return;
                        if (name === 'assistant' && !this.chatAvailable) return;
                        // The rail button of the open panel closes the sidebar;
                        // a forced call (picker, palette, inspector) always opens
                        if (!force && this.sidebar && this.rail === name) {
                            this.closePanel();
                            return;
                        }
                        this.rail = name;
                    ...
                    // server's gate AND the menu's switch. It is the rail's
                    // first panel.
                    ...
                    setAssistant(on) {
                        if (on) this.setRail('assistant', true);
                        else if (this.assistantOpen) this.closePanel();
                    },
                    ...
                    // The header's one responsive button steps Desktop → Tablet → Phone
                    cycleDevice() {
                    ...
                    toggleDevMode() {
                        ...
                        if (!this.devMode && this.rail === 'assistant') this.setRail('sections', true);
    ...
    <x-slot:menu>
        ...
        >
            ...
            <button @click="open = !open" class="s-rail-btn s-brand" :class="open && 'is-open'" title="Menu" aria-label="Menu" aria-haspopup="menu" :aria-expanded="open">
    ...
    {{-- ============================================================ --}}
    {{-- Sidebar — one panel, chosen by the rail                        --}}
    {{-- ============================================================ --}}
    <x-slot:sidebar>
